...
def decodeMessage(message):    
    ...
    return isAA, TTL, IP_decoded

def encodeMessage(hostname):
    ID = 12345
    QR = 0 #message is a query
    OPCODE = 0 # A standard query has opcode 0, but need to turn it to 0000 because 4 bit field
    AA = 0 #only valid in responses so I'm going to set it to 0
    TC = 0 #valid in responses only
    RD = 1 
    RA = 0 #set in responses
    Z = 0 #must be0 in all queries and responses
    RCODE = 0 #error code set in responses only
    QDCOUNT = 1 #asking 1 question which is tmz.com ip?
    ANCOUNT = 0 #filled by response: number of RR in answers
    NSCOUNT = 0 #Filled by response: number of name server resource records in authority records
    ARCOUNT = 0 #Filled by response: number of RRs in additional records section
...
def sendDNSreq(ip, message):
    ...
    try:
        ...
        sock.close()
        return data, startTimeRoot, endTimeRoot
def checkAndUpdateCache():
    if(os.path.exists('cache.txt') == False):
        createFile = open("cache.txt", 'w')
        createFile.close()
    with open('cache.txt.temp', 'w') as writeCache:
        with open('cache.txt', 'r') as cache:
            for line in cache:
                print(line)
                makeSplit = line.split(" ")
                if len(makeSplit)> 1:
                    dateString = " ".join((makeSplit[0], makeSplit[1]))
                    date1 = datetime.strptime(dateString.split(".")[0], "%Y-%m-%d %H:%M:%S")
                    date2 = datetime.now()
                    if(date1 > date2):
                        writeCache.write(line)
        cache.close()
    writeCache.close()

    os.remove('cache.txt')
    os.rename('cache.txt.temp', 'cache.txt')
...
startCacheSearch = time.time()
with open('cache.txt', 'r') as checkCache:
    for line in checkCache:
        decodedCacheLine = line.split(" ")
        if hostname == str(decodedCacheLine[2]):
            finalIP = str(decodedCacheLine[3])
            endCacheSearch = time.time()
            #print("IP RETRIEVED FROM CACHE", finalIP)
            #print("RTT for CACHE: " + str(endCacheSearch - startCacheSearch) + "\n")
checkCache.close()
...
if(finalIP == ""):
    ...
    #print("\nResponse raw:\n" + response)
    isAuthoritative, TTL, decodedResponse = decodeMessage(response)
    ...
    finalIP = decodedResponse
    print("TLD server IP address:", serverList[0])
    print("Authoritative server IP address:", serverList[1])

    now = datetime.now()
    timeExpired = now + timedelta(seconds = int(TTL))
    afterTTL = timeExpired.strftime("%m/%d/%Y %H:%M:%S ")
    #print(afterTTL)
    cacheStore = str(timeExpired) + " " + str(hostname) + " " + str(decodedResponse) + "\n"
    #print(cacheStore)
    cache = open("cache.txt", "a")
    cache.write(cacheStore)
    cache.close()

#print("THIS IS THE FINAL IP: ", finalIP)
print("HTTP server IP address:", finalIP)
