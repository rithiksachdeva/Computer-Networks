...
def accept(ip, srvport,rtj,PLP):
    ...
    while welcomeSocket[0] != "":
        ...
        if(msgtype == 'SYN'):
            ...
            thread = threading.Thread(target=connectionSocket, name="Conn-"+str(port), args=(connectsock,port,stop_event,))
            #start the thread
            thread.start()
    stop_event.set()
    thread.join()
    ...
    while True:
        ...
        if(src == clientPort1 and dest == connectServerPort and msgtype == 'DATA'):
            ...
            if(generatedplp > PLP):
                ...
                locallastackseq=lastackseq
                if seq > lastack:
                   if seq == lastackseq or lastackseq == 0:
                      writefile.write(data)
                      lastack = seq
                      lastackseq = int(seq) + len(data) + 16
                   else:
                      print("SKIP MISSING ACK DROP WRITE",src,dest,seq,ack,lastack,lastackseq,writefile.tell(),len(data))
                   locallastackseq=lastackseq
                else:
                   if seq < lastackseq:
                      locallastackseq = seq
                   print("DUPLICATE ACK DROP WRITE",src,dest,seq,ack,lastack,lastackseq,writefile.tell(),len(data))
                ackMsg = create_ack(dest,src,ack,locallastackseq)
                log(dest,src, 'ACK', len(ackMsg))
                if generatedrtj > rtj:
                    time.sleep(generatedrtj)
                    print("SENT ACK DELAY PACKET: RTJ",dest,src,ack,lastackseq,writefile.tell())
                else:
                    print("SENT ACK PACKET:",dest,src,ack,lastackseq,writefile.tell())
